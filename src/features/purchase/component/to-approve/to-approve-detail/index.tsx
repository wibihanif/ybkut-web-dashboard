...
import { ToApproveDetailTable } from './ToApproveDetailTable';
import { Link, useSearchParams } from 'react-router-dom';
import { useToApproveDetailSearchParams } from '~/features/purchase/hooks/useToApproveDetailSearchParams';
import { useDebouncedState } from '@mantine/hooks';
import { SortOrder } from '~/types/pagination';
import { useGetTotalApproveDetail } from '~/features/purchase/api/useGetTotalApproveDetail';
...
export const ToApproveDetail: React.FC = () => {
  ...
  const { data: toApproveDetail, isLoading: isLoadingToApproveDetail } = useGetTotalApproveDetail({
    page,
  ...
  const totalPage = Math.ceil((toApproveDetail?.meta?.total as number) / LIMIT_PER_PAGE);
  ...
  useToApproveDetailSearchParams({
  ...
  return (
    <Box>
      ...
      <Paper
        ...
        <ToApproveDetailTable
          handleSort={handleSort}
          isLoadingToApproveDetail={isLoadingToApproveDetail}
          page={page}
          setPage={setPage}
          sortBy={sortBy}
          sortOrder={sortOrder}
          totalPage={totalPage}
          toApproveDetail={toApproveDetail?.data || []}
        />
      </Paper>
    </Box>
  );
